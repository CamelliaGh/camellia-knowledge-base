...
Tip:
In Python, a class that contains only static methods is often a code smell. It usually means the class is being used for organization rather than modeling behavior.
...
Note:
This is called duck typing: if it walks like a duck and quacks like a duck, Python doesn’t care what it inherits from.
...
Warning:
If a class exists only to group functions, it’s probably unnecessary.
